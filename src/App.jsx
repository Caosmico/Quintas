...
const CircleOfFifths = () => {
  ...
  const [coltraneMode, setColtraneMode] = useState(false);
  ...
  const getNoteColor = (note, type) => {
    ...
    if (note === getRelative(displayNote, displayType)) {
      if ((displayType === 'major' && type === 'minor') || 
        ...
        return '#9b59b6'; // Púrpura para relativa
      }
    }
  ...
  const isInColtraneCycle = (note, type) => {
    if (!coltraneMode || !displayNote || type !== displayType) return false;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-8">
      <div className="bg-white rounded-2xl shadow-2xl p-8">
        ...
        {/* Botón para activar modo Coltrane */}
        <div className="flex justify-center mb-4">
          <button
            onClick={() => setColtraneMode(!coltraneMode)}
          ...
          >
            {coltraneMode ? '✓ Modo Coltrane Activo' : 'Activar Modo Coltrane'}
          </button>
        </div>
        ...
        <div className="flex justify-center gap-3 mb-4 flex-wrap">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full border-2 border-gray-700" style={{backgroundColor: '#4a90e2'}}></div>
            <span className="text-sm font-medium text-slate-700">I/i (Tónica)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full border-2 border-gray-700" style={{backgroundColor: '#e74c3c'}}></div>
            <span className="text-sm font-medium text-slate-700">V/v (Dominante)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full border-2 border-gray-700" style={{backgroundColor: '#2ecc71'}}></div>
            <span className="text-sm font-medium text-slate-700">IV/iv (Subdominante)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full border-2 border-gray-700" style={{backgroundColor: '#f39c12'}}></div>
            <span className="text-sm font-medium text-slate-700">ii/ii° (Supertónica)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full border-2 border-gray-700" style={{backgroundColor: '#3498db'}}></div>
            <span className="text-sm font-medium text-slate-700">vi/VI (Superdominante)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full border-2 border-gray-700" style={{backgroundColor: '#e67e22'}}></div>
            <span className="text-sm font-medium text-slate-700">iii/III (Mediante)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full border-2 border-gray-700" style={{backgroundColor: '#95a5a6'}}></div>
            <span className="text-sm font-medium text-slate-700">vii°/VII (Sensible)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full border-2 border-gray-700" style={{backgroundColor: '#9b59b6'}}></div>
            <span className="text-sm font-medium text-slate-700">Relativa</span>
          </div>
        </div>
        
        {/* Información dinámica */}
        <div className="text-center bg-slate-50 rounded-lg p-6 min-h-[180px] flex items-center justify-center">
              ...
              <p className="text-lg text-slate-700 mb-3">
                ...
                {pinnedNote && (
                  <span className="ml-2 text-xs bg-orange-100 text-orange-800 px-2 py-1 rounded">
                    Fijada (click para desfijar)
                  </span>
                )}
              </p>
              
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm text-slate-700 mb-3 max-w-xl mx-auto">
      ...
      <div className="mt-8 max-w-2xl bg-white rounded-xl shadow-lg p-6">
        <h2 className="text-2xl font-bold mb-4 text-slate-800">Grados de la Escala y Cualidades de los Acordes</h2>
        <div className="space-y-3 text-slate-600">
          ...
          <div className="bg-purple-50 border-l-4 border-purple-500 p-4 rounded-lg mt-4">
            <p className="font-bold mb-2 text-purple-900">Modo Coltrane - Ciclo de Terceras Mayores:</p>
            <p className="text-purple-800">
              El "Modo Coltrane" muestra el ciclo de terceras mayores, una técnica de improvisación 
              popularizada por John Coltrane en composiciones como "Giant Steps". 
            </p>
            <p className="text-purple-800 mt-2">
              En el círculo de quintas, las terceras mayores forman un triángulo equilátero. 
              Por ejemplo, desde C: E (tercera mayor) y A♭ (otra tercera mayor desde E) forman 
              este patrón geométrico.
            </p>
            <p className="text-purple-800 mt-2">
              <strong>Ejemplo desde C:</strong> C → E → A♭ → C (cada paso es una tercera mayor, 4 semitonos)
            </p>
            <p className="text-purple-700 mt-2 text-sm italic">
              💡 Activa el Modo Coltrane y selecciona una nota para ver su ciclo de terceras mayores 
              marcado con borde azul.
            </p>
          </div>
        </div>
      </div>
